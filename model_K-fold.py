...
class kappa_call(Callback):
    ...
    def __init__(self, filepath, validation_data=None, max_nums=1):
...
class Metrics2(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        y_val = get_labels(model, y_validation)
        y_pred = get_preds(model, x_validation)
...
x_train3,y_train3=myDataload_kaggle()
...
for train, validation in kfold.split(x, y):
    ...
    wts = class_weight.compute_class_weight('balanced', np.unique(y[train]),
                                        y[train])

    model = build_model2()
# model = my_InceptionResNetV2(input_shape=(IMG_SIZE, IMG_SIZE, 3),classes=5)
    model.compile(loss='mse', optimizer=Adam(lr=0.0005), metrics=['acc'])

    data_generator = create_datagen().flow(x[train],
                                       y[train],
    ...
    model_dir = os.path.join('.', '5_foldmodels', st)
    if not os.path.exists(model_dir):
        os.makedirs(model_dir)
    model_path = os.path.join(model_dir, model.name + '_{epoch:03d}_kappa-{kappa:.5f}.h5')

    kappa_callback = kappa_call(model_path, validation_data=(x[validation], y[validation]))
    ...
    history2 = model.fit_generator(
        ...
        class_weight=wts,
        validation_steps=x[validation].shape[0] // BATCH_SIZE,
